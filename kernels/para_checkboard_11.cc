...
float time1=get_cycles();
  ...
  for(n = 0;n < iter;n++) chess_prepare_for_pipelining{
    ...
    if(n==0){//first iteration
            ...
            for(int k=0;k<N1;k++){
                if(k%2==0){
                    xdnv[k]=upd_elem(xdnv[k],0,readincr(din2));
                    ydnv[0]= upd_elem(ydnv[0],k/2,readincr(din3));
                }else{
                    xdnv[k+32]=upd_elem(xdnv[k+32],0,readincr(din2)); 
                    ydnv[32]= upd_elem(ydnv[32],k/2,readincr(din3));    
                }
            }
    ...
    for(int k=0;k<N1;k++){ //the value of s effects the neighboring values
                    if(k%2==0){
                        writeincr(dout2,ext_elem(s[k+32],0));
                        writeincr(dout3,ext_elem(s[0],k/2));
                    }else{
                        writeincr(dout2,ext_elem(s[k],0));
                        writeincr(dout3,ext_elem(s[32],k/2));
                    }
                }
