...
float time1=get_cycles();
  ...
  for(n = 0;n < iter;n++) chess_prepare_for_pipelining{
    ...
    //following iterations effected by neighboring node
            for(int k=0;k<N1;k++){
                if(k%2==0){
                    xupv[k+32]=upd_elem(xupv[k+32],15,readincr(din2));
                    ydnv[0]= upd_elem(ydnv[0],k/2,readincr(din1));
                }else{
                    xupv[k]=upd_elem(xupv[k],15,readincr(din2)); 
                    ydnv[32]= upd_elem(ydnv[32],k/2,readincr(din1));    
                }
            }
    ...
    for(int k=0;k<N1;k++){ //the value of s effects the neighboring values
                    if(k%2==0){
                        writeincr(dout2,ext_elem(s[k+32],15));
                        writeincr(dout1,ext_elem(s[0],k/2));
                    }else{
                        writeincr(dout2,ext_elem(s[k],15));
                        writeincr(dout1,ext_elem(s[32],k/2));
                    }
                }
