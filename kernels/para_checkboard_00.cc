  ...
  for( n = 0;n < iter;n++) chess_prepare_for_pipelining{
    ...
    if(n==0){//first iteration
            ...
            for(int k = 0;k < VOLUME/16;k++) chess_prepare_for_pipelining{
                ...
                if (expand(-beta*sum)> 16*rand(seed)){ //update the neighboring blocks
                    s[k] = stmp;
    ...
    }else{//following iterations effected by neighboring node
            for(int k=0;k<N1;k++){
                if(k%2==0){
                    xupv[k+32]=upd_elem(xupv[k+32],15,readincr(din0));
                    yupv[63]= upd_elem(yupv[63],k/2,readincr(din1));
                }else{
                    xupv[k]=upd_elem(xupv[k],15,readincr(din0)); 
                    yupv[31]= upd_elem(yupv[63],k/2,readincr(din1));    
                }
            }
    ...
    for(int k=0;k<N1;k++){ //the value of s effects the neighboring values
                    if(k%2==0){
                        writeincr(dout0,ext_elem(s[k+32],15));
                        writeincr(dout1,ext_elem(s[63],k/2));
                    }else{
                        writeincr(dout0,ext_elem(s[k],15));
                        writeincr(dout1,ext_elem(s[31],k/2));
                    }
                }
