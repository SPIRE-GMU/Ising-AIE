...
inline void init_lfsrs(void){
    lfsr32_11 = 0xABCDE;
    lfsr31_11= 0x23456789;
}

inline int get_random(void){
    shift_lfsr(&lfsr32_11, POLY_MASK_32);
    return (shift_lfsr(&lfsr32_11, POLY_MASK_32)^ shift_lfsr(&lfsr31_11, POLY_MASK_31)) & 0xffff;
}
               ...
               v16int16 neighbours,stmp;
                ...
                if(k==VOLUME/16/2){ // update all the white nodes and communicate with the neighboring engines, before the following computing of black nodes 
                        ...
                        for(int kk=0;kk<16;kk++){
                            writeincr(dout3,ext_elem(s[0],kk)); //send white to AIE_01
                            writeincr(dout2,ext_elem(s[kk*2],0));   //send white to AIE_10                                           
                        }             
                        for(int kk=0;kk<16;kk++){
                            xdnv[32]=upd_elem(xdnv[32],kk,readincr(din3)); //accept white from AIE-01                             
                            ydnv[kk*2+32+1]=upd_elem(yupv[kk*2+32+1],0,readincr(din2));    //accept white from AIE-10                                          
                        }
                }
