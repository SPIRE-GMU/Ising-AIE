               ...
               v16int16 neighbours,stmp;
                ...
                if(k==VOLUME/16/2){ // update all the white nodes and communicate with the neighboring engines, before the following computing of black nodes 
                        ...
                        for(int kk=0;kk<16;kk++){
                            ...
                            yupv[63]=upd_elem(yupv[63],kk,readincr(din3));    //accept white from AIE-11                     
                            xdnv[kk*2+32+1]=upd_elem(xdnv[kk*2+32+1],0,readincr(din0));    //accept white from AIE-00                                          
                        }
                        
                        for(int kk=0;kk<16;kk++){
                            writeincr(dout0,ext_elem(s[k*2],0)); //send white to AIE_00
                            writeincr(dout3,ext_elem(s[31],kk));   //send white to AIE_11                                           
                        }             
