...
inline void init_lfsrs(void){
    lfsr32_10 = 0xABCDE;
    lfsr31_10= 0x23456789;
}

inline int get_random(void){
    shift_lfsr(&lfsr32_10, POLY_MASK_32);
    return (shift_lfsr(&lfsr32_10, POLY_MASK_32)^ shift_lfsr(&lfsr31_10, POLY_MASK_31)) & 0xffff;
}
               ...
               v16int16 neighbours,stmp;
                ...
                if(k==VOLUME/16/2){ // update all the white nodes and communicate with the neighboring engines, before the following computing of black nodes 
                        ...
                        for(int kk=0;kk<16;kk++){
                            ...
                            ydnv[32]=upd_elem(ydnv[32],kk,readincr(din1));    //accept white from AIE-00                     
                            xupv[kk*2]=upd_elem(xupv[kk*2],15,readincr(din2));    //accept white from AIE-11                                         
                        }
                        
                        for(int kk=0;kk<16;kk++){
                            writeincr(dout1,ext_elem(s[0],kk)); //send white to AIE_00
                            writeincr(dout2,ext_elem(s[kk*2],15));   //send white to AIE_11                                           
                        }             
