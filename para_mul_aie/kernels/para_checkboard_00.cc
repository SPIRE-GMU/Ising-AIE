...
inline void init_lfsrs(void){
    lfsr32_00 = 0xABCDE;
    lfsr31_00= 0x23456789;
}

inline int get_random(void){
    shift_lfsr(&lfsr32_00, POLY_MASK_32);
    return (shift_lfsr(&lfsr32_00, POLY_MASK_32)^ shift_lfsr(&lfsr31_00, POLY_MASK_31)) & 0xffff;
}
...
void para_checkboard_00(   input_stream<int16> *din0,output_stream<int16> *dout0, input_stream<int16> *din1,output_stream<int16> *dout1,input_buffer<int32> &start ){
    ...
    float time4=get_cycles();
    printf("start board 00:\n");
    auto *start_flag =start.data();
    int32_t a1 = *start_flag++;
    int32_t a2 = *start_flag;    
    printf("continue board 00:\n");
    ...
    for(i = 0;i < VOLUME;i++) chess_prepare_for_pipelining{
      if(i%50==0)printf("initialize seed, i=%d:\n",i);
      if(rand(seed) < 0.5){
    ...
    // for(int v=0;v<8;v++)
    //  printf("s[2]=%d\n",ext_elem(s[2],v)); 
    printf("start initialize index board 00:\n");
    ...
    printf("finish initialize index board 00:\n");
  ...
  for( n = 0;n < iter;n++) chess_prepare_for_pipelining{
    ...
    v16int32 esumv;
            ...
            for(int k = 0;k < VOLUME/16;k++) chess_prepare_for_pipelining{
               /*---------------8 para intrinsics-----------------*/
               v16int32 new_energy,energy_now,deltae;
               // v16int16 stmp;
               // v16int16 sv;
               v16int16 neighbours,stmp;
               
               if(k==VOLUME/16/2){ // update all the white nodes and communicate with the neighboring engines, before the following computing of black nodes 
                   
                        for(int kk=0;kk<16;kk++){
                            writeincr(dout0,ext_elem(s[k*2],15)); //send white to AIE_01
                            writeincr(dout1,ext_elem(s[31],kk));   //send white to AIE_10                                           
                        }             
                        for(int kk=0;kk<16;kk++){
                            xupv[kk*2+32]=upd_elem(xupv[kk*2+32],15,readincr(din0)); //accept white from AIE-01                             
                            yupv[63]=upd_elem(yupv[63],kk,readincr(din1));    //accept white from AIE-10                                          
                        }
                }
               ...
               energy_now = lsrs(mul(stmp,neighbours),0); // extract v16 because mul32 is not permitted
               new_energy = lsrs(mul(s[k],neighbours),0);
               // if(n==2 && k==0){
    ...
    for(int v=0;v<16;v++){
            ...
            mag = mag + ext_elem(lsrs(magv,0),v);
    }
